...
const countArrayNum = (arr) => {
    let returnArr = new Array(Math.max(...arr) + 1).fill(0);
    for (let item of arr) {
        returnArr[item]++;
    }
    return returnArr;
...
const analogCounter2 = (string1, string2) => {
    let freqObj = {};
...
const grabUniqueItem = (arr) => {
    if (arr.length === 0)
        return [];
    let totalUnique = new Array();
    let leftPointer = 0;
    let rightPointer = 1;
...
const sameFrequency = (num1, num2) => {
    const getNum1String = num1.toString();
    const getNum2String = num2.toString();
    let newObject = {};
    // console.log('checked');
    if (getNum1String.length !== getNum2String.length)
        return false;
...
const areThereDuplicatePractice = (arr) => {
    let objCount = {};
    for (let i = 0; i < arr.length; i++) {
        let currVal = arr[i];
...
const averagePairPractice = (arr, average) => {
    let leftPointer = 0;
    let rightPointer = arr.length - 1;
    while (leftPointer < rightPointer) {
...
const isSubsequencePractice = (string1, string2) => {
    let length = 0;
    for (let i = 0; i < string2.length; i++) {
        if (string1[length] === string2[i])
            length++;
    }
...
const maxSubArraySumPractice = (arr, max) => {
    if (arr.length < max)
        return null;
    let maxSum = 0;
    let tempSum = 0;
...
const minSubArrayLenPractice = (arr, len) => {
    let minLen = Infinity;
    let leftPointer = 0;
    let rightPointer = 0;
    let sum = 0;
...
const findLongestSubstringPractice = (string) => {
    let longestLen = 0;
    let stringObj = {};
    let left = 0;
    for (let i = 0; i < string.length; i++) {
